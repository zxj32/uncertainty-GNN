...
checkpt_file = 'data/tem_model/{}_{}_1.ckpt'.format(FLAGS.dataset, FLAGS.model)
cost_val = []
acc_val = []
val_loss_min = np.inf
patience_step = 0
...
for epoch in range(FLAGS.epochs):
    ...
    # if epoch > 350 and cost_val[-1] < np.mean(cost_val[-(FLAGS.early_stopping + 1):-1]):
...
Baye_result = []
for p in range(100):
    feed_dict = construct_feed_dict_un_teacher(features, support, y_test, test_mask, epoch, teacher_probability, placeholders)
    feed_dict.update({placeholders['dropout']: FLAGS.dropout})
