...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'amazon_electronics_photo', 'Dataset string.')  # 'amazon_electronics_computers' 'amazon_electronics_photo' 'ms_academic_phy'
flags.DEFINE_string('model', 'S_BGCN_T', 'Model string.')  # 'S_BGCN_T'
...
checkpt_file = 'data/tem_model/{}_{}_2.ckpt'.format(FLAGS.dataset, FLAGS.model)
cost_val = []
acc_val = []
val_loss_min = np.inf
patience_step = 0
...
for epoch in range(FLAGS.epochs):
    ...
    feed_dict = construct_feed_dict_un_teacher(features, support, y_train, train_mask, epoch, teacher_probability,
                                               placeholders)
    ...
    # Validation
    cost, acc, duration = evaluate(features, support, y_val, val_mask, epoch, teacher_probability, placeholders)
    cost_val.append(acc)
...
#  MC-dropout to sample parameter, we choose 500 samples and take average
Baye_result = []
for p in range(50):
    feed_dict = construct_feed_dict_un_teacher(features, support, y_val, val_mask, epoch, teacher_probability,
                                               placeholders)
    feed_dict.update({placeholders['dropout']: FLAGS.dropout})
